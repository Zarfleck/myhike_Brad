...
function displayCardsDynamically(collection) {
    ...
    db.collection(collection).limit(3).orderBy("length").get()   //the collection called "hikes"
        .then(allHikes => {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                ...
                newcard.querySelector('i').onclick = () => upadteBookmark(docID);
...
//-----------------------------------------------------------------------------
function upadteBookmark(hikeDocID) {
    // Check if this hikeID had been bookmarked or not
    // 1- get the list of the bookmarks 
    currentUser.get().then(userDoc=>{
        let bookmarksNow = userDoc.data().bookmarks;
        console.log(bookmarksNow)

        if(bookmarksNow.includes(hikeDocID)){
            console.log("This hikeID exists")
            currentUser.update({
            ...
            console.log("This hikeID should be added")
        }
    })
